function kmlStruct = kml2struct_multi(kmlFile, dimensions)
    ...
    for ii = 1:Nos
        ...
        % Find value field.
        bucket2 = regexp(objectStrings{ii},'<SimpleData name="ElevTop_ft".*?>.+?</SimpleData>','match');
        % Clip off flags
        coordStr2 = regexprep(bucket2{1},'<SimpleData.*?>(\s+)*','');
        coordStr2 = regexprep(coordStr2,'(\s+)*</SimpleData>','');
        % Split coordinate string by commas or white spaces, and convert string
        % to doubles
        Value = str2double(regexp(coordStr2,'[,\s]+','split'));
        ...
        for geo_ii = 1:geo_Nos
            ...
            kmlStruct(geo_counter).Value = Value;
